...
export class CartService {
  ...
  add(item: MenuItem) {
    ...
    if (found) found.quantity += 1; else lines.push({ item, quantity: 1 });
    this._lines.set(lines);
  }

  remove(itemId: number) { this._lines.set(this._lines().filter(l => l.item.id !== itemId)); }
  changeQty(itemId: number, qty: number) {
    this._lines.set(this._lines().map(l => l.item.id === itemId ? { ...l, quantity: Math.max(1, qty) } : l));
  }
